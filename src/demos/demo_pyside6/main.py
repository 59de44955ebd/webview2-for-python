...
from PySide6.QtCore import Qt, QResource, __version__
from PySide6.QtGui import QPalette, QColor, QActionGroup
from PySide6.QtWidgets import QApplication, QMainWindow, QMessageBox, QFileDialog

APP_NAME = 'WebView2 Demo PySide6'
...
from webview2.embed.pyside6 import *
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # loadUI() is an utility function provided by webview2.pyside6 that
        # allows to load an .ui file into the current class instance, just
        # like in PyQt5/6.
        loadUi(os.path.join(APP_DIR, 'main.ui'), self)
...
#                self.webview.profile_apply_theme(PREFERRED_COLOR_SCHEME.DARK if is_dark else PREFERRED_COLOR_SCHEME.LIGHT)
    ...
    def set_theme(self, action):
        if action == self.action_theme_auto:
            ...
            is_dark = action == self.action_theme_dark
            qApp.styleHints().setColorScheme(Qt.ColorScheme.Dark if is_dark else Qt.ColorScheme.Light)
            if self.webview.webview_ready:
                self.webview.profile_apply_theme(PREFERRED_COLOR_SCHEME.DARK if is_dark else PREFERRED_COLOR_SCHEME.LIGHT)
    ...
    def about(self):
        QMessageBox.about(
            ...
            'About',
            (
                f'<b>{APP_NAME}</b><br><br>A simple demo based on Python, PySide6 and WebView2.<br><br>'
                f'Python version: {sys.version.split()[0]}<br>'
                f'PySide6 version: {__version__}<br>'
